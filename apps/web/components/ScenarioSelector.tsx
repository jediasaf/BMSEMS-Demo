...
export function ScenarioSelector({
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-1">
        {scenarios.map((scenario) => (
          <button
            ...
            className={cn(
              'focus-ring inline-flex items-center gap-1.5 rounded-panel border px-2 py-1 text-2xs font-medium transition-colors',
              selected === scenario.scenario_id
                ? 'border-accent/50 bg-accent/12 text-accent'
                : 'border-base-500 bg-base-700/60 text-ink-300 hover:border-base-400 hover:text-ink-100',
            )}
          >
      ...
      {injection && (
        <div className="flex flex-wrap items-center gap-2 rounded-panel border border-prov-injected/40 bg-prov-injected/[0.07] px-2 py-1.5">
          <Pill tone="neutral" className="border-prov-injected/50 text-prov-injected">
            Injected scenario
          </Pill>
          <span className="tabular text-2xs text-ink-200">
          ...
          <span className="ml-auto font-mono text-[10px] text-ink-500">seed {injection.seed}</span>
        </div>
      )}

      {detail && active && (
        <div className="rounded-panel border border-base-600 bg-base-850/70 p-2.5">
          <div className="text-2xs font-semibold text-ink-100">{active.name}</div>
          <div className="text-[10px] uppercase tracking-wider text-ink-500">{active.subtitle}</div>
